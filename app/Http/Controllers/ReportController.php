    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $datatable =  Datatables::of($products)
                ...
                ->editColumn('pieces', function ($row) {
               
                        $pieces = $row->pieces ? $row->pieces : 0;

                        return  '<span class="current_pieces" data-orig-value="' . (float)$pieces . '" data-unit="' . "Pcs" . '"> ' . $pieces . '</span>' . ' ' . "Pcs";
                ...
                ->editColumn('total_pieces_sold', function ($row) {
                    $total_pieces_sold = 0;
                    if ($row->total_pieces_sold) {
                        $total_pieces_sold =  (float)$row->total_pieces_sold;
                    }

                    return  '<span class="current_pieces" data-orig-value="' . (float)$total_pieces_sold . '" data-unit="' . "Pcs" . '"> ' . $total_pieces_sold . '</span>' . ' ' . "Pcs";
                })
            ...
            $raw_columns  = ['unit_price', 'total_transfered', 'total_sold',
                    'total_adjusted', 'stock', 'pieces', 'stock_price', 'stock_value_by_sale_price', 'potential_profit','total_pieces_sold'];
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($activities)
                            ...
                            ->addColumn('subject_type', function ($row) use ($transaction_types) {
                                ...
                                } elseif ($row->subject_type == 'App\User') {
                                    $subject_type = __('report.user');
                                } elseif ($row->subject_type == 'App\Transaction' && !empty($row->subject->type)) {
