        ...
        if (!empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                ...
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['pieces'] = $purchase_line->pieces;
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                ...
                                $qty_remaining = $this->productUtil->num_uf(trim($v['quantity']));
                                    ...
                                    $new_pieces = $this->productUtil->num_uf(trim($v['quantity']));
                                ...
                                if (isset($v['purchase_line_id'])) {
                                    ...
                                    if ($qty_remaining != 0) {
                                        ...
                                        $this->productUtil->updateProductQuantity($location_id, $product->id, $k, $qty_remaining, $old_qty, null, false, $product->pieces);
                                    }
                                } else {
                                    if ($qty_remaining != 0) {
                                        $product->update(['pieces' => $new_pieces]);
