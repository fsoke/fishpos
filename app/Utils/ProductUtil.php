...
class ProductUtil extends Util
{
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            $pieces = $this->num_uf($data['pieces']);
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                Product::where('id',$data['product_id'])->update(['pieces'=>$pieces]);
                ...
                if ($transaction->status == 'received') {
                    $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details,$pieces);
                }
            }
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        $products = $query->select(
            ...
            'p.id as product_id',
            'p.pieces as pieces',
            'p.total_pieces_sold as total_pieces_sold',
            'units.short_name as unit',
